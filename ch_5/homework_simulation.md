...
simulator that shows how processes are related in a single “familial”
tree. Read the relevant README for details about how to run the
simulator.
...
A: Yes, whenever `a` forks, it creates a branch. 

>2. One control the simulator gives you is the fork percentage, controlled
> by the `-f` flag. The higher it is, the more likely the next
...
>think the resulting final process trees will look like as the percentage
...
A: Yes, whenever a new branch is created its parent forked it. Whenever it leaves,
that process exited.
...
>use the `-R` flag? Learn more about what happens to orphaned processes
> on your own to add more context.

A: I thought the child processes would also exit, but the child processes of
 `c` end up staying and becoming children of `a`. Learning more about it,
 whenever a parent process exits and has children, it will become the child of
 the `init` process, which in this case was process `a`.

>6. One last flag to explore is the `-F` flag, which skips intermediate
...
a:
  ...
  c:  
```
>7. Finally, use both `-t` and `-F` together. This shows the final process
...
>In which cases can you tell? In which can’t you tell? Try some different
> random seeds to delve into this question.
